<#
.SYNOPSIS
	Export all MFA details about users in the environment.

.DESCRIPTION
	This script used to output all MFA information about the users in the environment. It will output a csv that contains the following collumns:
    DisplayName | UserPrincipalName | MFAStatus | ActivationStatus | DefaultMFAMethod |	AllMFAMethods |	MFAPhone |	MFAEmail |	LicenseStatus |	IsAdmin | AdminRoles | SignInStatus


.PARAMETERS 
    DisabledOnly - output only disabled users 
    EnabledOnly - output only active users
    EnforcedOnly - output only mfa in enforced status
    ConditionalAccessOnly - output only MFA enforced via Conditional Access policy
    AdminOnly - output information only about Admin accounts
    LicensedUserOnly - output only licensed users

.EXAMPLE
	Get-UserMFADetails.ps1 -AdminOnly 
