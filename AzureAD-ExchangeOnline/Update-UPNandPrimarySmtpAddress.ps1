...
# ex: After 365 -> 365 migration cutover, mailboxes need to have those properties changed from contoso.onmicrosoft.com -> contoso.com
...
foreach ($m in $mailboxes) {
    $newUPN = $m.UserPrincipalName.Split("@")[0] + "@" + $domain    # remove old domain, append new domain
