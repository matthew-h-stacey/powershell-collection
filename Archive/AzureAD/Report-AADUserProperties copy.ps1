...
# Currently uses a combination of AzureAD module, Teams module, and an "App registration" in Azure for Graph API to query properties about either an AzureAD User or Group
...
Write-Host "Connecting to AzureAD, check for background popup window for Authentication"
Connect-AzureAD
...
if ($Group -ne '') { # if group parameter was provided
    ...
    if ($EnabledOnly){ # if option parameter "EnabledOnly" is specified,
        ...
        $feed = Get-AzureADGroupMember -ObjectId (Get-AzureADGroup -Filter "startswith(DisplayName,'$Group')").ObjectID -All:$true -ErrorAction Stop | Sort-Object DisplayName # get all users of group, includes deleted
    }
}
