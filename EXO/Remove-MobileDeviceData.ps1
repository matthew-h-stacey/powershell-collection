...
function Remove-MobileDeviceData {
    ...
    foreach ($UPN in $UserPrincipalName) {
        ...
        Write-Host "[EXO] Attempting to locate $($UPN)'s mobile devices to delete data from..."
        ...
        if ($null -eq $userPhones) {
            ...
            foreach ($p in $userPhones) {    
                ...
                Write-Host "[EXO] Initiated email account wipe from mobile device: $($p.Name),$($p.DeviceModel)"
            }
        }        
    }
...
function Remove-MobileDevicePartnerships {
    ...
    foreach ($UPN in $UserPrincipalName) {
        ...
        if ($null -eq $userPhones) {
            ...
            Write-Host "[EXO] Found mobile device(s). Removing the account from $($UPN)'s mobile device(s)"
            foreach ($p in $userPhones) {    
                #Remove-MobileDevice -Identity $p.DistinguishedName -Confirm:$False 
                Write-Host "[EXO] Removed mobile device partnership: $($p.Name),$($p.DeviceModel)"
            }
        }
