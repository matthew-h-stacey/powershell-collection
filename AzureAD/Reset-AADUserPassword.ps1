...
if ($Random.IsPresent) {
    ...
    $pwLength = 32
    Write-Host "Randomly generating a $($pwLength)-character password"
    $pw = Get-RandomPassword($pwLength)
    if ($pw.Length -ge $pwLength) { # verify random password was generated successfully
        Write-Host "Setting password on user $($UserPrincipalName) to randomly generated password"
        Set-AzureADUserPassword -ObjectId $UserPrincipalName -Password (ConvertTo-SecureString "$pw" -AsPlainText -Force)
    }   
    else { Write-Host "[ERROR] Issue occured during random password generation. Please try again" }
