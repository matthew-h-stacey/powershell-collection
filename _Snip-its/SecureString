# Notes for creating and using a SecureString in a script instead of plain text credentials
# Relies on Windows DAPI. Only works under the user account that the SecureString was created as

# < PRE-SCRIPT WORK, START >
 
# Create a secure string using password as input
$password = Read-Host "Enter password" -AsSecureString
# Convert the secure string to an encrypted standard string
$encryptedSS = ConvertFrom-SecureString -SecureString $password
$encryptedSS # output the encrypted string to console to use it as the input for the ConvertTo-SecureString object

# < PRE-SCRIPT WORK, END >

# < SCRIPT, START >

# Example, pretending the long string output of $encrypyptedSS is as shown below, create $securePassword to use in your script with that input
$username = "\CONTOSO\svcaccount"
$securePassword = ConvertTo-SecureString ""
$creds = New-Object System.Management.Automation.PSCredential -ArgumentList $username, $securePassword
New-PSDrive -Name "Reports" -PSProvider "FileSystem" -Root "\\FILE01\IT\Reports" -Credential $creds
Do-Stuff

# < SCRIPT, END >